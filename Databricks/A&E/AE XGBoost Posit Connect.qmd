...
write_xlsx(future_forecast_output, outfile)

cat("File exists:", file.exists(outfile), "\n")
}
...
if (combine_results) {
  ...
  forecast_dir <- "Databricks/ae outputs"
  ...
  if (length(files) > 0) {
    ...
    combined_forecast <- purrr::map_dfr(
      files,
      ~ readxl::read_excel(.x),
      .id = "source_file"
    )
    ...
    cat("No forecast files found to combine\n")
...
for (f in files) {
  cat(sprintf("- [%s](Databricks/ae outputs/%s)\n", f, f))
}
```
